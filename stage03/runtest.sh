...
for img in $winimgs; do
  while [ $retry -gt 0 ]; do
	...
	if [ -z "$(echo $ret | tail -n 1 | grep 'SUCCESS')" ]; then
       ...
       ret=$(./ephemeral.sh)
       if [ -z "$(echo $ret | tail -n 1 | grep 'SUCCESS')" ]; then
	       echo "ERROR: ephemeral disks test failed ($ret)";
	       exitCode=1
               ret=$(./eucalog.sh)
	       echo "WINDOWS INSTANCE LOG: $ret"
       else
	       echo "Passed ephemeral disk test"
       fi
